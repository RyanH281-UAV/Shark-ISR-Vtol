...
- [ ] `colcon build --packages-select shark_isr_interfaces shark_isr_autopilot` passes in WSL
- [ ] `ros2 launch shark_isr_autopilot autopilot.launch.py` starts cleanly against SITL
- [ ] Verify offboard setpoints + loiter/orbit in SITL
- [ ] Verify failsafe: link loss → PX4 RTL/hold (companion not in the safety loop)
...
- [ ] `colcon build --packages-select shark_isr_guidance` passes in WSL
- [ ] SITL end-to-end: inject SetGuidanceMode SEARCH → vehicle executes pattern
- [ ] SITL: inject mock Detection → vehicle transitions to TRACK orbit
...
- [ ] `colcon build --packages-select shark_isr_perception` passes in WSL
- [ ] SITL end-to-end: launch perception (sim) → mock Detection published → guidance receives
...
- [ ] `colcon build --packages-select shark_isr_mission` passes in WSL
- [ ] SITL end-to-end: CMD_START → ARM → OFFBOARD → TRANSIT → SEARCH sequence
- [ ] SITL: CMD_ABORT → RTL verified
- [ ] SITL: low battery trigger test
