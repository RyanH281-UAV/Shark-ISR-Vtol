...
import SensorPiP from "@/components/SensorPiP";
import { STATE_COLOR, type State } from "@/lib/guidance";
import { useInViewport } from "@/lib/useInViewport";
import type { Telemetry } from "./MissionCanvasPro";

const MissionCanvasPro = dynamic(() => import("./MissionCanvasPro"), {
  ssr: false,
...
const EMPTY_TEL: Telemetry = { state: "TRANSIT", conf: 0, sustain: 0, frameHit: false, geoloc: null };
...
export default function MissionSection3D() {
  const [tel, setTel] = useState<Telemetry>(EMPTY_TEL);
  ...
  return (
    ...
    >
      ...
      {/* caption rail — tracks the active state */}
      ...
      {/* detection HUD — confidence integrator, τ gate, K_SUSTAIN, geoloc */}
      <div className="absolute bottom-4 left-4 z-10">
        <DetectionHUD t={tel} variant="panel" />
      </div>

      {/* sensor PiP — drone's-eye view with YOLO box */}
      ...
      >
        {webgl === false ? (
          ...
          <CanvasBoundary fallback={<StaticFallback />}>
            <MissionCanvasPro onTelemetry={setTel} showCallouts={true} />
          </CanvasBoundary>
        ) : (
